...
#show heading: it => if (it.depth == 1) {
    set align(center)
    show h.where(amount: .3em): none
    text(size: 15pt, it)
} else { text(size: 13pt, it) }
// #show ref.where(form: "normal"): set ref(supplement: it => {
//     if it.func() == heading { if it.level == 1 { "实验" } else { "" } } else if it.func() == figure { "Figure" }
// })
...
#let zebraw = zebraw.with(numbering-separator: true, radius: 10pt, lang: false)
