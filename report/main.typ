...
#{
    ...
    let _commit-hash-table-rows(path: "commit/hash.txt") = {
        ...
        for line in read(path).split("\n") {
            ...
            let exp-no = exp.replace("exp", "").replace("part", ".")
...
#let zebraw-file(filepath, part: none) = context {
    let heading-no = counter(heading).get().at(0, default: 0)
    let raw-dir = if part == none {
        "raw/" + str(heading-no)
    } else {
        "raw/" + str(heading-no) + "/part" + str(part)
    }
    let raw-path = raw-dir + "/" + _raw-file-key(filepath) + ".txt"
...
=== 实现
#{
    let zebraw-part-file = zebraw-file.with(part: 1)
    [存储层接口:]
    zebraw-part-file("src/observer/storage/db/db.h")
    zebraw-part-file("src/observer/storage/db/db.cpp")
    [语句层支持:]
    zebraw-part-file("src/observer/sql/stmt/drop_table_stmt.h")
    zebraw-part-file("src/observer/sql/stmt/drop_table_stmt.cpp")
    zebraw-part-file("src/observer/sql/stmt/stmt.cpp")
    [执行层支持:]
    zebraw-part-file("src/observer/sql/executor/drop_table_executor.h")
    zebraw-part-file("src/observer/sql/executor/drop_table_executor.cpp")
    zebraw-part-file("src/observer/sql/executor/help_executor.h")
...
=== 实现
#{
    let zebraw-part-file = zebraw-file.with(part: 2)
    [类型枚举与注册:]
    zebraw-part-file("src/observer/common/type/attr_type.h")
    zebraw-part-file("src/observer/common/type/attr_type.cpp")
    zebraw-part-file("src/observer/common/type/data_type.cpp")
    [类型行为实现(4字节 *YYYYMMDD* 存储,比较,字符串化,解析校验,闰年):]
    zebraw-part-file("src/observer/common/type/date_type.h")
    zebraw-part-file("src/observer/common/type/date_type.cpp")
    [Value封装:]
    zebraw-part-file("src/observer/common/value.h")
    zebraw-part-file("src/observer/common/value.cpp")
    [隐式类型转换:]
    zebraw-part-file("src/observer/common/type/char_type.cpp")
    [词法与语法支持:]
    zebraw-part-file("src/observer/sql/parser/lex_sql.l")
    zebraw-part-file("src/observer/sql/parser/yacc_sql.y")
    [比较与编码支持:]
    zebraw-part-file("src/observer/sql/optimizer/physical_plan_generator.cpp")
    zebraw-part-file("src/observer/sql/expr/expression.cpp")
