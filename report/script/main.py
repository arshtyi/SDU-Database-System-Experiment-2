...
def run_git(*args: str) -> str:
    try:
        ...
        cmd = " ".join(exc.cmd)
        detail = f"{cmd} failed with exit code {exc.returncode}"
        if stderr:
            detail += f": {stderr}"
...
def resolve_branch_ref(branch: str) -> str:
    ...
    for ref in candidates:
        try:
            ...
            continue
    raise RuntimeError(
        f"Cannot resolve branch '{branch}'. Checked refs: {', '.join(candidates)}. "
        "Ensure CI fetches experiment branches (e.g. exp2..exp5)."
    )
