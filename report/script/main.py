...
COMMIT_SUBJECT_PATTERN = re.compile(
    r"^add:\s*experiment\s+(\d+)(?:\s*-\s*part\s*(\d+))?\.?$",
    re.IGNORECASE,
)
...
def clear_existing_raw_tree(target_dir: Path) -> None:
...
def latest_experiment_commits(branch: str) -> dict[int, dict[int | None, tuple[str, str]]]:
    commits: dict[int, dict[int | None, tuple[str, str]]] = {}
    ...
    for line in run_git("log", branch_ref, "--format=%H%x09%s").splitlines():
        ...
        part_no = int(match.group(2)) if match.group(2) else None
        commits.setdefault(exp_no, {}).setdefault(part_no, (commit, subject))
    return commits


def final_experiment_commit(
    commits: dict[int, dict[int | None, tuple[str, str]]],
    exp_no: int,
    branch: str,
) -> tuple[str, str]:
    exp_commits = commits.get(exp_no)
    if not exp_commits:
        raise RuntimeError(
            f"Cannot find commit with prefix 'add: experiment {exp_no}' in branch {branch}"
        )

    part_numbers = sorted(part for part in exp_commits if part is not None)
    if part_numbers:
        return exp_commits[part_numbers[-1]]
    if None in exp_commits:
        return exp_commits[None]

    raise RuntimeError(
        f"Cannot find final commit for experiment {exp_no} in branch {branch}"
    )


def collect_branch_diff_targets(branch: str) -> list[RawDiffTarget]:
    ...
    old_commit, old_subject = final_experiment_commit(commits, prev_exp_no, branch)
    exp_commits = commits.get(exp_no)
    if not exp_commits:
        raise RuntimeError(
            f"Cannot find commit with prefix 'add: experiment {exp_no}' in branch {branch}"
    ...
    targets: list[RawDiffTarget] = []
    part_numbers = sorted(part for part in exp_commits if part is not None)
    if part_numbers:
        for part_no in part_numbers:
            new_commit, new_subject = exp_commits[part_no]
            targets.append(
                RawDiffTarget(
                    label=f"{branch} part {part_no}",
                    target_dir=RAW_ROOT / str(exp_no) / f"part{part_no}",
            ...
            old_commit, old_subject = new_commit, new_subject
    elif None in exp_commits:
        new_commit, new_subject = exp_commits[None]
        targets.append(
            RawDiffTarget(
                label=branch,
                target_dir=RAW_ROOT / str(exp_no),
                old_commit=old_commit,
                new_commit=new_commit,
                old_subject=old_subject,
                new_subject=new_subject,
    ...
    if not targets:
        raise RuntimeError(f"Cannot collect raw diff targets for branch {branch}")
    return targets
...
def generate_raw_diff_files(branches: list[str], unified: int) -> None:
    for branch in branches:
        ...
        targets = collect_branch_diff_targets(branch)
        clear_existing_raw_tree(exp_dir)
        for target in targets:
            ...
            for file_diff in file_diffs:
                write_raw_file(target.target_dir, file_diff)

            print(
                f"{target.label}: {target.old_commit[:8]}..{target.new_commit[:8]} -> "
                f"{len(file_diffs)} files at {target.target_dir.relative_to(REPO_ROOT)} "
                f"({target.old_subject} -> {target.new_subject})"
            )
...
def collect_branch_hash_rows(branch: str) -> list[CommitHashRow]:
    exp_no = branch_exp_number(branch)
    branch_ref = resolve_branch_ref(branch)
    commits = latest_experiment_commits(branch)
    exp_commits = commits.get(exp_no)
    if not exp_commits:
        _, commit = latest_branch_commit(branch)
        return [CommitHashRow(label=branch, branch_ref=branch_ref, commit=commit)]
    ...
    if part_numbers:
        return [
            CommitHashRow(
                ...
                commit=exp_commits[part_no][0],
            )
            for part_no in part_numbers
        ]

    commit, _ = final_experiment_commit(commits, exp_no, branch)
    return [CommitHashRow(label=branch, branch_ref=branch_ref, commit=commit)]
...
def generate_commit_hash_file(
    ...
    lines: list[str] = []
    for branch in branches:
        for row in collect_branch_hash_rows(branch):
            lines.append(f"{row.label},{row.branch_ref},{row.commit}")
